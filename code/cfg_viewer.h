...
Agnode_t *make_graph_node(graph_t *g, std::string content, std::string comments, bool has_branch = false) {
...
void make_cfg(Array<Basic_Block *> basic_blocks) {
    ...
    for(auto bb : basic_blocks) {
        ...
        bool has_branch = (bb->insts.len != 0) && (bb->insts[bb->insts.len-1]->type == INST_BRANCH);
        ...
        for(auto parent_bb : bb->preds) {
            ...
            if (parent_bb->insts.len != 0) {
                ...
                if (has_branch) {
                    ...
                    type = (br_inst->true_target == bb) ? TRUE_TARGET : FALSE_TARGET;
                }
            }
