...
Phi *has_phi(Basic_Block *bb) {
    ...
    if (first) return (Phi *)bb->insts[0];

    for(auto v : bb->insts) {
        if (v->type == PHI) return (Phi *)v;
    }
...
struct IR_Generator {
    ...
    uint32 insert(Basic_Block *basic_block, Value *v, bool insert_to_first = false);

    // IR emitting
    Procedure_IR *emit_procedure(Ast_Procedure *proc_ast);
    Value *emit_value(Ast_Node *node, Ast_Block *ast_block);
    Value *emit_offset(Ast_Array_Reference *ref, Type_Info_Array *array_info, Ast_Block *ast_block);
    void emit_stmts(Ast_Statement *stmt, Ast_Block *ast_block);
    Function_Call *emit_function_call(Ast_Procedure_Call *call_node, bool32 has_return_value, Ast_Block *ast_block);
    Block_And_Branch* short_circuit(Ast_Node* root, Basic_Block* left, Basic_Block* right, bool first_call = true);
    Basic_Block *emit_cond_expr(Ast_Node *cond, Basic_Block *true_target, Basic_Block *false_target, Ast_Block *ast_block, bool is_loop_header = false);

    void save_variable(Ast_Declaration *decl, Value *value, Basic_Block *belongs_to_block);
    ...
    Value *add_phi_operands(Ast_Declaration *decl, Phi *phi, Basic_Block *block);
...
Block_And_Branch* IR_Generator::short_circuit(Ast_Node* root, Basic_Block* left, Basic_Block* right, bool first_call) {
    if (first_call) condition_blocks.release();
    if (root->type == Ast_Type_Binary_Op)
    {
        ...
        auto condition_less = short_circuit((Ast_Node*)temp->rhs, left, right, false);
        if (temp->op == BINARY_LOGICAL_AND) {
            condition_great = short_circuit((Ast_Node*)temp->lhs, left, condition_less->first, false);
            returnvalue->second->false_target = left;
            returnvalue->second->true_target = right;
 
        }
        if (temp->op == BINARY_LOGICAL_OR) {
            condition_great = short_circuit((Ast_Node*)temp->lhs, condition_less->first, right, false);
            returnvalue->second->false_target = left;
            returnvalue->second->true_target = right;
...
Basic_Block *IR_Generator::emit_cond_expr(Ast_Node *cond, Basic_Block *true_target, Basic_Block *false_target, Ast_Block *ast_block, bool is_loop_header) {
    ...
    while (length != 0) {
        ...
        while(l!=0) {
            ...
            if (temp->type == INST_BRANCH) {
                connect_CFG(k->first, ((Instruction_Branch*)temp)->true_target);
                connect_CFG(k->first, ((Instruction_Branch*)temp)->false_target);
                break;
           }
            l--;
       }
...
Function_Call *IR_Generator::emit_function_call(Ast_Procedure_Call *call_node, bool32 has_return_value, Ast_Block *ast_block) {
...
void IR_Generator::emit_stmts(Ast_Statement *stmt, Ast_Block *ast_block) {
    ...
    for(; stmt; stmt=stmt->next) {
        ...
        switch(node->type) {
            ...
            case Ast_Type_Integer_Literal: {} break;
            ...
            case Ast_Type_Assignment: {
                ...
                Value *value_to_assign = emit_value(assign_node->value_to_assign, ast_block);
                if (assign_node->lhs->type == Ast_Type_Array_Reference) {
                    auto ref = (Ast_Array_Reference *)assign_node->lhs;
                    auto decl_node = find_declaration(ast_block, ref->array_id, &program_ast->globals);

                    Value *base = read_variable(decl_node, bb);
                    Type_Info_Array *arr_info = (Type_Info_Array *)decl_node->decl_type;
                    ...
                    mem_write->value_to_write = new_use(value_to_assign, (Value *)mem_write);

                    insert(bb, mem_write);

                } else {
                    value_to_assign->comment = print_ast_for_ir((Ast_Node *)assign_node);
                    auto lhs_id = (Ast_Identifier *)assign_node->lhs;
                    auto decl_node = find_declaration(ast_block, lhs_id, &program_ast->globals);

                    if (decl_node->is_global) {
                        auto global_address = read_variable(decl_node, bb);
                        auto mem_write = new Memory_Write;
                        mem_write->decl = decl_node;
                        mem_write->comment = print_ast_for_ir((Ast_Node *)decl_node->id);
                        mem_write->base = new_use(global_address, (Value *)mem_write);
                        mem_write->value_to_write = new_use(value_to_assign, (Value *)mem_write);
            ...
            case Ast_Type_While: {
                ...
                //creat some empty block for loop unrolling one occur after emit cond expr ,another is in tail prepared for creating another while
                Basic_Block* empty1 = new Basic_Block(f);
                Basic_Block* empty2 = new Basic_Block(f);

                bb = header;
                header = emit_cond_expr(while_node->condition, empty1, empty2, ast_block, true);
            ...
            case Ast_Type_If: {
                ...
                auto if_bb = new Basic_Block,
                     then_bb = new Basic_Block;
                ...
                Basic_Block *if_bb_exit = bb,
                            *then_bb_exit = then_bb;

                // emit then block
                if (ast_if->else_block) {
                    bb = then_bb;
                    emit_stmts(ast_if->else_block->first_statement, ast_if->else_block);
                    then_bb_exit = bb;
                }
                ...
                if (if_bb_exit || then_bb_exit) {
                    ...
                    if (if_bb_exit) connect_CFG(if_bb_exit, if_end, true);
                    if (then_bb_exit) connect_CFG(then_bb_exit, if_end, true);
            ...
            default: {
                exit(30);
                report_error("Internal Compiler Error: translating unsupported language construct to IR.");
            }
...
Value *IR_Generator::emit_value(Ast_Node *node, Ast_Block *ast_block) {
    if (node->type == Ast_Type_Binary_Op) {
        ...
        binary_inst->lhs = new_use(emit_value(binary_node->lhs, ast_block), (Value *)binary_inst);
        binary_inst->rhs = new_use(emit_value(binary_node->rhs, ast_block), (Value *)binary_inst);
        ...
        if (decl->is_const && decl->decl_type->tag != TYPE_ARRAY) {
            auto constant = evaluate(ast_block, decl->initial_value, program_ast->globals);
        ...
        if (decl->initial_value) {
            ...
            for(auto init_inst : init_seq) {
                ...
                if (init_inst.tag == ZEROS) {
                    ...
                    auto num_of_bytes = new Constant(init_inst.num_of_zeros*4);
                    insert(bb, (Value *)num_of_bytes);

                    memset_call->arguments.push(new_use(base, memset_call));
                    memset_call->arguments.push(new_use(byte_to_set, memset_call));
                    memset_call->arguments.push(new_use(num_of_bytes, memset_call));
                    ...
                    mem_write->value_to_write = new_use(emit_value(init_inst.init_value, ast_block), mem_write);
        ...
        auto array_decl = find_declaration(ast_block, ref->array_id, &program_ast->globals);
...
Value *IR_Generator::emit_offset(Ast_Array_Reference *ref, Type_Info_Array *array_info, Ast_Block *ast_block) {
    ...
    int pitch = 1;
    for(int32 i = 0; i < DECL_DIMS; i++) pitch *= decl_dims[i];
...
uint32 IR_Generator::insert(Basic_Block *basic_block, Value *v, bool insert_to_first) {
...
void IR_Generator::save_variable(Ast_Declaration *decl, Value *value, Basic_Block *block) {
...
Value *IR_Generator::read_variable_recursively(Ast_Declaration *decl, Basic_Block *block) {
...
Value *IR_Generator::add_phi_operands(Ast_Declaration *decl, Phi *phi, Basic_Block *block) {
...
void remove_trivial_phi(Procedure_IR *f) {
    ...
    while(!phi_worklist.empty()) {
        auto phi = phi_worklist.back(); phi_worklist.pop();

        // already optimized out
        if (phi->b == NULL) continue;

        Value *same = NULL;
        for(auto op_use : phi->operands) {
            Value *op = op_use->v;
            if (op == same || (op == (Value *)phi)) continue;
            if (same != NULL) goto dont_optimize_phi;
            same = op;
        }
        ...
        for(auto opr : phi->operands) opr->remove();
...
Value *try_remove_trivial_phi(Phi *phi, Ast_Declaration *decl) {
    ...
    for (auto user : users) {
        if (auto next_phi = user->as<Phi>()) {
            auto optimized_value = try_remove_trivial_phi(next_phi, next_phi->decl); // @TODO: swap problem?

            if (optimized_value != user) {
                // replace all defs in blocks
                for(auto it = next_phi->decl->value_map.begin();
                        it != next_phi->decl->value_map.end();
                        it++) {
...
void insert_br(Basic_Block *from, Basic_Block *to) {
    ...
    direct_br->target = to;
    direct_br->b = from;
    /*assert(from->insts.len == 0 || (from->insts.back()->type != INST_DIRECT_BRANCH &&
            from->insts.back()->type != INST_BRANCH));*/
...
Branch_Condition binary_op_to_branch_cond(Binary_Op_Type op_type) {
    switch(op_type) {
        case BINARY_LESS_THAN:                { return LESS_THAN; } break;
        case BINARY_GREATER_THAN:             { return GREATER_THAN; } break;
        case BINARY_LESS_THAN_OR_EQUAL_TO:    { return LESS_THAN_OR_EQUAL; } break;
        case BINARY_GREATER_THAN_OR_EQUAL_TO: { return GREATER_THAN_OR_EQUAL; } break;
        case BINARY_NOT_EQUAL_TO:             { return NOT_EQUAL; } break;
        case BINARY_EQUAL_TO:                 { return EQUAL; } break;
        default: { exit(31); report_error("Internal Compiler Error: converting unknown binary op type to branch type."); }
    }
    return NO_CONDITION;
}
        ...
        switch(v->type) {
            ...
            case INST_BINARY: {
                ...
                switch(bi_inst->op_type) {
                    case BINARY_ADD:                      { p << '+'; } break;
                    case BINARY_SUBTRACT:                 { p << '-'; } break;
                    case BINARY_MULTIPLY:                 { p << '*'; } break;
                    case BINARY_DIVIDE:                   { p << '/'; } break;
                    case BINARY_MOD:                      { p << "%"; } break;
                    case BINARY_LESS_THAN:                { p << lt; } break;
                    case BINARY_GREATER_THAN:             { p << gt; } break;
                    case BINARY_LESS_THAN_OR_EQUAL_TO:    { p << le; } break;
                    case BINARY_GREATER_THAN_OR_EQUAL_TO: { p << ge; } break;
                    case BINARY_NOT_EQUAL_TO:             { p << "!="; } break;
                    case BINARY_EQUAL_TO:                 { p << "=="; } break;
                    case BINARY_LOGICAL_OR:               { p << logical_or; } break;
                    case BINARY_LOGICAL_AND:              { p << "&&"; } break;
                    case BINARY_LSL:                      { p << lsl; } break;
                    case BINARY_LSR:                      { p << rsl; } break;
                    default: { exit(19); assert(false && "converting unsupported binary operator to IR"); }
                }
                p << " v" << bi_inst->rhs->v->n;
            ...
            case INST_UNARY: {
                ...
                p << 'v' << v->n << " := "  ;
                switch (bi_inst->op_type) {
                case UNARY_POSITIVE: { p << ' '; } break;
                case UNARY_NEGATIVE: { p << '-'; } break;
                case UNARY_NOT:      { p << '!'; } break;
                default: { exit(20); assert(false && "converting unsupported unary operator to IR"); }
                }
            ...
            case INST_BRANCH: {
                ...
                p << "br v" << br_inst->cond->v->n << " ? B" << br_inst->true_target->index_in_procedure << " : B" << br_inst->false_target->index_in_procedure;
            } break;
            ...
            case MEMORY_DEF: {
                ...
                if (def->is_initial_version) {
                    ...
                    assert(def->clobbers);
                    p << " := " << "v" << def->cause->v->n << " clobbers v" << def->clobbers->v->n;
                }
            ...
            case FUNCTION_CALL: {
                ...
                for(int i = 0; i < call->arguments.len; i++) {
                    ...
                    if (i != call->arguments.len - 1) p << ", ";
                }

                p << ")";
            } break;

            default: { exit(32); report_error("Internal Compiler Error: printing unknown IR instrution"); }
        }
...
Array<Use *> get_operands(Value *v) {
    ...
    switch(v->type) {
        ...
        case MEMORY_DEF: { 
            ...
            if (mdef->clobbers) ops.push(mdef->clobbers);
            if (mdef->cause) ops.push(mdef->cause);
        } break;

        case MEMORY_READ: { 
            auto mread = (Memory_Read *)v;
            if (mread->base) ops.push(mread->base);
            if (mread->offset) ops.push(mread->offset);
            if (mread->mem_ver) ops.push(mread->mem_ver);
        } break;

        case MEMORY_WRITE: { 
            auto mwrite = (Memory_Write *)v;
            if (mwrite->base) ops.push(mwrite->base);
            if (mwrite->offset) ops.push(mwrite->offset);
            if (mwrite->value_to_write) ops.push(mwrite->value_to_write);
        } break;
...
bool32 has_branch(Basic_Block *bb) {
    ...
    if (bb->insts.len != 0) {
        auto last_inst = bb->insts[bb->insts.len-1];
        if (last_inst->type == INST_RETURN || last_inst->type == INST_BRANCH || last_inst->type == INST_DIRECT_BRANCH) {
            return true;
        }
    }
    return false;
...
bool is_relational_binary_op(Binary_Op_Type op) {
    return (op == BINARY_LESS_THAN ||
            op == BINARY_LESS_THAN_OR_EQUAL_TO ||
            op == BINARY_GREATER_THAN ||
            op == BINARY_GREATER_THAN_OR_EQUAL_TO ||
            op == BINARY_EQUAL_TO ||
