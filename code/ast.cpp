...
bool is_literal_0(Ast_Node *n) {
    if (n->type != Ast_Type_Integer_Literal) return false;
...
Array<Init_Inst> get_init_sequence(Array<int32> dims, Ast_Init_List *init_list) {
    ...
    uint64 one_d = 1;
    int DIM = dims.len;
    for(int i = 0; i < DIM; i++) one_d *= dims[i];
    ...
    while(1) {
        Ast_Init_List *init = init_lists.back(); init_lists.pop();
        int prev_index = list_indices.back(); list_indices.pop();
        size_t len = init->list.len;
        uint64 i;
        for(i = prev_index; i < len; i++) {
            if (init->list[i]->type == Ast_Type_Init_List) {
                uint64 level = init_lists.len;
                list_indices.push(i+1); list_indices.push(0);
                init_lists.push(init); init_lists.push((Ast_Init_List *)init->list[i]);
        ...
        if (i == len) { // reaches the end of init list
            uint64 need = fill_to.back(); fill_to.pop(); need_to_init.pop();
            if (index + 1 == need) {
...
void print_ast(String_Builder *s, Ast_Node *n, bool is_cfg_viewer) {
    ...
    switch(n->type) {
        ...
        case Ast_Type_Binary_Op: {
            ...
            switch(bi_node->op) {
                case BINARY_ADD:                      { s->append("+"); } break;
                case BINARY_SUBTRACT:                 { s->append("-"); } break;
                case BINARY_MULTIPLY:                 { s->append("*"); } break;
                case BINARY_DIVIDE:                   { s->append("/"); } break;
                case BINARY_MOD:                      { s->append("%%"); } break; // NOTE that we are using sprintf... sigh...
                case BINARY_LESS_THAN:                { s->append(lt); } break;
                case BINARY_GREATER_THAN:             { s->append(gt); } break;
                case BINARY_LESS_THAN_OR_EQUAL_TO:    { s->append(le); } break;
                case BINARY_GREATER_THAN_OR_EQUAL_TO: { s->append(ge); } break;
                case BINARY_NOT_EQUAL_TO:             { s->append("!="); } break;
                case BINARY_EQUAL_TO:                 { s->append("=="); } break;
                case BINARY_LOGICAL_OR:               { s->append(logical_or); } break;
                case BINARY_LOGICAL_AND:              { s->append("&&"); } break;
				case BINARY_LSL:					  { s->append("<<");  }break;
				case BINARY_LSR:					  { s->append(">>");  }break;
				case BINARY_ASL:					  { s->append("<<");  }break;
				case BINARY_ASR:					  { s->append(">>");  }break;
				case BINARY_RSB: // aha?
                default: { assert(false && "printing unknown AST binary op"); }
            }
        ...
        case Ast_Type_Unary_Op: {
            ...
            switch(unary_node->op) {
                case UNARY_NEGATIVE: { s->append("-"); } break;
                case UNARY_POSITIVE: {} break;
                case UNARY_NOT:      { s->append("!"); } break;
                default: { assert(false && "printing unknown AST unary op"); }
            }
        ...
        case Ast_Type_Procedure_Call: {
            ...
            for(int i = 0; i < call_node->arguments.len; i++) {
                ...
                if (i != call_node->arguments.len-1) { s->append(", "); }
            }
            s->append(")");

        } break;

        default: { assert(false && "can't print this type of AST node"); }
