...
OPT_PASS(compute_callgraph) {
    ...
    for(auto f : IR->procedures) {
        for(auto bb : f->blocks) {
            for(auto v : bb->insts) {
                ...
                if (f->callee.find(callee) == -1) f->callee.push(callee);
                if (callee->caller.find(f) == -1) callee->caller.push(f); 
            }
        }
    }
    ...
    // naive solution:
    // if a function doesn't issue memory writes and reads and doesn't call another
    // function with side effects, it's then pure
                            ...
                            if (call->f == f) {
                                for(auto u=call->use; u;) {
                                    ...
                                    if (auto def = u->user->as<Memory_Def>()) {
                                        def->replace_and_remove(def->clobbers->v);
                                        removed_def = true;
                                    }
                                    u = nxt;
                                }
    ...
    for(auto f : IR->procedures) {
        printf("%s:", f->name);
        if (f->has_side_effect) { printf(" has side effects\n"); }
        /*
