...
bool is_constexpr(Ast_Node *n) {
    if (n->type == Ast_Type_Identifier) return true;
            ...
            for(auto bb : f->blocks) {
                for(auto I : bb->insts) {
                    if (auto mem_write = I->as<Memory_Write>()) {
                        ...
                        // alias problem
                        // an global array can be passed to an function, which might modified its content
                        ...
                        for(int i = 0; i < call->arguments.len; i++) {
                            ...
                            // we don't care whether that function modifies the content in the pointer or not
                            // just mark the global variable
                            if (glo_ref) {
                                printf("potential aliasing for %s, marking as variable\n", glo_ref->decl->id->name);
        ...
        for(auto f : prog->procedures) {
            for(auto bb : f->blocks) {
                for(int i = 0; i < bb->insts.len; i++) {
                    ...
                    if (!mem_read) continue;

                    auto glo_val = mem_read->base->v->as<Global>();
                    if (!glo_val) continue;

                    if (is_variable[glo_val->decl]) continue;

                    if (!is_constexpr(mem_read->source)) continue;

                    int const_to_be_baked = evaluate(NULL, mem_read->source, prog->globals);
...
OPT_PASS(bake_constant) {
    BAKE_CONST::find_const_globals(IR);
}
