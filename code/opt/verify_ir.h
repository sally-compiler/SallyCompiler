...
void verify_bb(Procedure_IR *f) {
    if (f->start_block->preds.len != 0) exit(60);
    if (f->exit_block->succs.len != 0) exit(61);
    for(auto bb : f->blocks) {
        ...
        if (bb != f->exit_block) {
            ...
            if (last->type != INST_BRANCH &&
                last->type != INST_DIRECT_BRANCH &&
        ...
        for(auto v : bb->insts) {
            if (auto p = v->as<Phi>()) {
                for(auto opr : p->operands) {
                    if (opr->v->type == GLOBAL) exit(202);
                    if (opr->v->type == ALLOCA) exit(203);
                    if (opr->v->type == MEMORY_WRITE) exit(204);
                    if (opr->v->b == NULL) exit(201);
                    if (opr->v->n > max_n) exit(200);
                    if (opr->v->n == -1)  exit(100);
                    if (opr->v->n < -1)   exit(99);
                }
            }
        }
    }
    ...
    for(auto bb : f->blocks) {
        for(auto v : bb->insts) {
            for(auto use : get_operands(v)) {
                auto opr = use->v;
                if (opr->b == NULL) exit(50);
                if (f->blocks.find(opr->b) == -1) {
                    if (opr->type == PHI) exit(55);
                    else exit(51);
                }
            }
        }
    ...
    for(auto bb : f->blocks) {
        for(auto v : bb->insts) {
            for(auto u=v->use; u; u=u->next) {
                if (u->v != v) assert(false);
                if (u->user->b == NULL) exit(53);
                if (f->blocks.find(u->user->b) == -1) exit(54);
            }
        }
    }
}
...
OPT_PASS(verify_ir) {
    run_on_every_function(IR, verify_function);
}
