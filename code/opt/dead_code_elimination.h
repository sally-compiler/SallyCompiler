...
namespace DCE {
    ...
    void dce_function(Procedure_IR *proc_ir) {
        ...
        for(int i = 0; i < NB-1; i++) dominated_by[NB-1][i] = 0;
        ...
        for (int i = 0; i < NB; i++) {
            ...
            for (int j = 0; j < NB; j++) {
                ...
                bool not_strictly_dominated_by_i = (i == j) || (!dominated_by[j][i]);
        ...
        */
        ...
        for(int i = 0; i < live_marks.len; i++) { live_marks[i] = false; }
        ...
        for(auto BB : proc_ir->blocks) {
            for(auto I : BB->insts) {
                ...
                if (I->type == FUNCTION_CALL) { // TODO: only mark function call that has side effects live
                    live_values.push(I);
                }
            }
        }
        ...
        while(!live_values.empty()) {
            ...
            auto v = live_values.back(); live_values.pop();
            live_marks[v->n] = true;
            ...
            if (v->type == PHI || v->type == MEMORY_PHI) {
                for(auto pred : v->b->preds) {
                    ...
                    if ((last_inst->type == INST_DIRECT_BRANCH || last_inst->type == INST_BRANCH) && !live_marks[last_inst->n]) {
                        live_values.push(last_inst);
                    }
                }
            }

            if (v->type == CONSTANT) continue;
        ...
        while(!live_queue.empty()) {
            auto bb = live_queue.front(); live_queue.pop();
            live_blocks.push(bb);
