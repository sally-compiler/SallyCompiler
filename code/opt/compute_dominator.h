...
namespace DOM {
    ...
    Array< Array<bool> > compute_dominator_matrix(Procedure_IR *func_IR, bool compute_post_dominator=false) {

        int NB = func_IR->blocks.len; // number of basic blocks

        auto dominated_by = make_2d<bool>(NB, NB);

        Basic_Block *top = (compute_post_dominator ? func_IR->exit_block : func_IR->start_block);
        ...
        for(int i = 0; i < NB; i++) dominated_by[top_i][i] = 0;
        ...
        while(changed) {
            ...
            for(int i = NB-1; i >= 0; i--) {
                if (i == top_i) continue;
                for(int j = 0; j < NB; j++) temp[j] = 1;

                Array<Basic_Block *> bbs = (compute_post_dominator ? 
                        (func_IR->blocks[i]->succs) : (func_IR->blocks[i]->preds));
                for(auto bb : bbs) {
    ...
    void compute_dominator_on_single_function(Procedure_IR *func_IR) {
        ...
        for(auto bb : func_IR->blocks) {
            if (!bb->imm_dom) {
                printf("B%d(depth %d) is dominated by:", bb->index_in_procedure, bb->dom_depth);
            } else {
                printf("B%d(imm_dom B%d, depth %d) is dominated by:", bb->index_in_procedure, bb->imm_dom->index_in_procedure, bb->dom_depth);
            }
