std::map<Basic_Block*, Array<Basic_Block*>> find_dominator(Procedure_IR* Graph) {
...
;				if (v->preds.len > 0) {
					...
					for (auto k : dom[v]) {//this compare the diffrernt may occur error!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
void DFS(Basic_Block * next, Map<Basic_Block*, Array<Basic_Block*>> dom, Procedure_IR* proc_ir,bool assign_or_findbackedge=true,bool clean=true) {
	if (clean) {
		index = 0;
		visited.clear();
	}
	if(assign_or_findbackedge)num[next] = index++;
	visited[next] = 1;
		...
		if (!assign_or_findbackedge) {
			if (num[block] < num[next] && visited[block] == 1 ) {
				if (dom[next].find(block) != -1) {
					...
					for (int i = 0; i < visited.len; i++) { visited[i] = false; }
					...
					while (!live_queue.empty()) {
						...
						for (auto pred : bb->preds) {
							if (!visited[pred->index_in_procedure] && pred!=block) {
			...
			for (auto val : block->insts) {
				if (val->invariant == true) {
					...
					case  INST_BINARY: {
						...
						if (k->rhs->v->type == CONSTANT || k->rhs->v->invariant ==true) {
							if (k->lhs->v->type == CONSTANT || k->lhs->v->invariant == true) {
								val->invariant = true;
								invariant.push(val);
								change = true;
								break;
						...
						else if (k->lhs->v->type == CONSTANT || k->rhs->v->invariant == true) {
							...
							if (native_loop->second.find(k->rhs->v->b) == -1 && native_loop->second.find(k->lhs->v->b) == -1) {
								val->invariant = true;
								invariant.push(val);
								change = true;
								break;
					...
					case INST_UNARY: {
						...
						if (k->oprend->v->type == CONSTANT || k->oprend->v->invariant == true) {
							k->invariant = true; change = true;
							invariant.push(val);
							break;
						}
...
Array<Basic_Block*> get_dominate_exit(Natural_Loop* native_loop, Procedure_IR* proc_ir) {
...
void invariant_code_motion(Procedure_IR* proc_ir ) {
	...
	for (auto native_loop : loops) {
		...
		for (auto val:invariant){
			//condition 1 :statement s dominate all exits,except condition block
			...
			//condition 2 :not assign to x,and I not sure my algorithm  is right
			...
			if (val_dominate_exits) {
				...
				while (curr)
				{
					if (curr->user->type == PHI) {
						...
						}//condition 3:all use in loop of x can be reach by the def x in s
						if (not_assign) {
							Use* c = p->use;
							while (c) {
				...
				//motion code
				if (not_assign) {
				 printf("v%d in block%d\n ",val->n,val->b->index_in_procedure);
				native_loop->second[0]->preds[0]->insts.push(val);
				printf("move v%d to block%d\n ", val->n, native_loop->second[0]->preds[0]->index_in_procedure);
				printf("remove v%d from block%d\n ", val->n, val->b->index_in_procedure);
				val->b->insts.remove(val);
				printf("edit v%d->b to block%d\n ", val->n, native_loop->second[0]->preds[0]->index_in_procedure);
