...
bool may_alias_by_dims(Ast_Declaration *x, Ast_Declaration *y) {
    ...
    if (x->decl_type->tag == TYPE_INTEGER || y->decl_type->tag == TYPE_INTEGER) {
        return false;
    }
    ...
    if (x_dims.len != y_dims.len) return false;

    for(int i = 0; i < x_dims.len; i++) {
        if (i == 0 && (x_dims[0] == -1 || y_dims[0] == -1)) continue;
        if (x_dims[i] != y_dims[i]) return false;
    }
...
bool may_alias(Ast_Declaration *x, Ast_Declaration *y) {
    ...
    // the whole memory
    if (x->decl_type == NULL || y->decl_type == NULL) return true;
...
void save_memory_version(Procedure_IR *f, Ast_Declaration *decl, Basic_Block *block, Value *d);
Value *read_memory_version(Procedure_IR *f, Ast_Declaration *decl, Basic_Block *block);

Array<Ast_Declaration *> current_live_decls;

Value *add_phi_operands(Procedure_IR *f, Ast_Declaration *decl, Memory_Phi *phi, Basic_Block *block) {
...
Value *read_memory_version(Procedure_IR *f, Ast_Declaration *decl, Basic_Block *block) {
    ...
    assert(ret_val);
    if (block != f->start_block) save_memory_version(f, decl, block, ret_val);

    return ret_val;
}

Value *read_memory_version_bb(Procedure_IR *f, Ast_Declaration *decl, Basic_Block *block) {
    Value *ret_val = NULL;
    auto vermap = f->bb_vermap[block];
    for(int i = vermap.len-1; i >= 0; i--) {
        auto it = vermap[i];
...
void save_memory_version(Procedure_IR *f, Ast_Declaration *decl, Basic_Block *block, Value *d) {
    ...
    if (current_live_decls.find(decl) == -1) current_live_decls.push(decl);
}
...
void visit_block(Procedure_IR *f, Basic_Block *bb) {
    ...
    for(int i = 0; i < bb->insts.len; i++) {
        ...
        if (auto store = v->as<Memory_Write>()) {
            ...
            def->clobbers = new_use(read_memory_version(f, store->decl, bb), def);
            def->decl = store->decl;
...
void build_function_mem_ssa(Procedure_IR *f) {
    ...
    while(!queue.empty()) {
        auto bb = queue.front(); queue.pop();

        if (bb_visited(bb)) continue;
