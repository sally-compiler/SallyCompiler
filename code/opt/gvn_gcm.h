...
namespace GVN_GCM {
    ...
    Basic_Block *find_LCA(Basic_Block *a, Basic_Block *b) {
        if (a == NULL) return b;
        while(a->dom_depth > b->dom_depth) a = a->imm_dom;
        while(b->dom_depth > a->dom_depth) b = b->imm_dom;
        while(a != b) {
            a = a->imm_dom;
            b = b->imm_dom;
        }
    ...
    // We only move constants, binary and unary insts, mem read/write, alloca and function calls
    ...
    bool cmp_value_numbering(const Value *a, const Value *b) {
        return a->n < b->n;
    }
    ...
    void schedule_late(Procedure_IR *func_IR, Value *v) {
        ...
        if (lca == NULL) { return; } // v is dead and isn't used anywhere
        ...
        //  (which can be v->b itself, thus altering the original algorithm a little bit)
    ...
    void schedule_value_within_bb(Value *v, Basic_Block *bb, Array<Value *> &new_insts) {
        if (v->visited) return;
        if (v->b != bb) return;
        v->visited = true;
        for(auto opr : get_operands(v)) {
    ...
    void gcm(Procedure_IR *func_IR) {
        ...
        for(auto bb : func_IR->blocks) {
            ...
            for(int i = 0; i < bb->insts.len; i++) {
                auto v = bb->insts[i];
                if (is_pinned(v)) { // pinned instructions remain in their original block
                    v->visited = true;
                    for(auto opr : get_operands(v)) {
                        schedule_early(func_IR, opr->v);
                    }
    ...
    VN get_vn(Value *v) {
        ...
        switch(v->type) {
            ...
            case INST_BINARY: {
                ...
                ss << "B " << "v" << lhs_n << "v" << b->op_type << "v" << rhs_n;
    ...
    int32 eval_value(Value *v) {
        if (auto bi = v->as<Instruction_Binary>()) {
            ...
            int32 l = eval_value(bi->lhs->v),
                  r = eval_value(bi->rhs->v);
            
            uint32 ul = l;

            switch(bi->op_type) {
                case BINARY_ADD:                      return l + r;
                case BINARY_SUBTRACT:                 return l - r;
                case BINARY_MULTIPLY:                 return l * r;
                case BINARY_DIVIDE:                   return l / r;
                case BINARY_MOD:                      return l % r;
                case BINARY_LSL:                      return ul << r;
                case BINARY_LSR:                      return ul >> r;
                case BINARY_NOT_EQUAL_TO:             return l != r;
                case BINARY_EQUAL_TO:                 return l == r;
                case BINARY_LESS_THAN_OR_EQUAL_TO:    return l <= r;
                case BINARY_GREATER_THAN_OR_EQUAL_TO: return l >= r;
                case BINARY_LESS_THAN:                return l < r;
                case BINARY_GREATER_THAN:             return l > r;
                default:              assert(false);
            }
            ...
            switch(un->op_type) {
                case UNARY_NOT:      return !opr;
                case UNARY_NEGATIVE: return -opr;
                case UNARY_POSITIVE: return +opr;
                default:             assert(false);
            }
    ...
    void number_value(Procedure_IR *f, Value *v) {
        ...
        auto call = v->as<Function_Call>();
        if (call && (call->f == NULL || call->f->has_side_effect)) return;

        // GVN those memory loads with memory version 
        auto load = v->as<Memory_Read>();
        if (load && load->mem_ver == NULL) return;
        ...
        */
        ...
        if (v->type == PHI || v->type == MEMORY_PHI) {
            ...
            for(auto opr : phi->operands) {
                ...
                if (same != NULL) return; // phi is non-trivial, return directly
                same = op;
            }
    ...
    void move_constant_to_rhs(Procedure_IR *f) {
        for (auto bb : f->blocks) {
            for (auto v : bb->insts) {
                if (auto bi = v->as<Instruction_Binary>()) {
                    if (bi->lhs->v->type != CONSTANT) continue;
                    if (bi->rhs->v->type == CONSTANT) continue;
    ...
    void gvn_gcm_on_single_function(Procedure_IR *f) {
        ...
        */
        ...
        for(auto bb : f->blocks) put_phis_to_start(bb);
