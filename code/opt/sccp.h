...
namespace SCCP {
    ...
    void visit_phi(Phi *phi) {
        ...
        if (cells[phi->n].state != new_cell.state || cells[phi->n].value != new_cell.value) {
    ...
    void visit_expression(Value *v) {
        if (v->type == INST_DIRECT_BRANCH) {
            ...
            switch(v->type) {
                case CONSTANT: { new_cell = {CONST, ((Constant *)v)->value}; } break;
                ...
                case FUNCTION_CALL:
                case ALLOCA:
                case MEMORY_READ: { new_cell = Variable; } break;

                // values that aren't values
                case MEMORY_WRITE:
                case INST_RETURN: break;

                // already handled
                case PHI:
                case INST_BRANCH:
                case INST_DIRECT_BRANCH: assert(false); break;
                ...
                case INST_UNARY: {
                    ...
                    if (opr_cell.state == CONST) {
                        ...
                        switch(unary_inst->op_type) {
                            case UNARY_NOT:      { n = !opr_cell.value; } break;
                            case UNARY_NEGATIVE: { n = -opr_cell.value; } break;
                ...
                case INST_BINARY: {
                    ...
                    bool32 both_const = (lhs_cell.state == CONST && rhs_cell.state == CONST);
                    if (both_const) {

                        int32 l = lhs_cell.value,
                              r = rhs_cell.value,
                              n; // new value

                        switch(bi_inst->op_type) {
                            case BINARY_ADD:                      { n = l + r; } break;
                            case BINARY_SUBTRACT:                 { n = l - r; } break;
                            case BINARY_MULTIPLY:                 { n = l * r; } break;
                            case BINARY_DIVIDE:                   { n = l / r; } break;
                            case BINARY_MOD:                      { n = l % r; } break;
                            case BINARY_EQUAL_TO:                 { n = l == r; } break;
                            case BINARY_NOT_EQUAL_TO:             { n = l != r; } break;
                            case BINARY_LESS_THAN_OR_EQUAL_TO:    { n = l <= r; } break;
                            case BINARY_GREATER_THAN_OR_EQUAL_TO: { n = l >= r; } break;
                            case BINARY_LESS_THAN:                { n = l < r; } break;
                            case BINARY_GREATER_THAN:             { n = l > r; } break;
                            case BINARY_LOGICAL_OR:               { n = l || r; } break;
                            case BINARY_LOGICAL_AND:              { n = l && r; } break;
                            case BINARY_LSL:                      { n = l << r; } break;
                            case BINARY_LSR:                      { n = l >> r; } break;
                    ...
                    } else if (lhs_cell.state == CONST || rhs_cell.state == CONST) { // if one of them is constant
            ...
            if (old_cell.state != new_cell.state || old_cell.value != new_cell.value) {
                cells[v->n] = new_cell;
    ...
    void prop_on_single_function(Procedure_IR *func_IR) {
        ...
        while(!flow_worklist.empty() || !ssa_worklist.empty()) {
            ...
            if (!flow_worklist.empty()) {
                Flow_Edge flow_edge = flow_worklist.front(); flow_worklist.pop();
                bool32 executed = (executable_flag.find(flow_edge) != executable_flag.end());
                ...
                auto use = ssa_worklist.front(); ssa_worklist.pop();
                ...
                if (use->user->type == PHI) {
                    ...
                    for(auto pred : bb->preds) {
                        ...
                        can_be_executed = (executable_flag.find(edge) != executable_flag.end());
                        if (can_be_executed) { break; }
                    }
