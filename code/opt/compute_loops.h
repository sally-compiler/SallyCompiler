...
namespace LOOP {
    ...
    // A loop is found
    void dfs(Procedure_IR *f, Basic_Block *bb, Array<int> &num, Array<Loop *> &loops, int i = 0) {
        if (bb_visited(bb)) return;
        ...
        for(auto pred : bb->preds) {
            ...
            if (bb_visited(pred) && num[pred->index_in_procedure] > num[bb->index_in_procedure]) {
    ...
    void compute_loop_body(Procedure_IR *f, Loop *l) {
        ...
        printf("loop (entry, exit) = (B%d, B%d) has body (", l->header->index_in_procedure, l->exit->index_in_procedure);
        for(auto bb : l->body) {
    ...
    void compute_loop_depth(Procedure_IR *f, Array<Loop *> &loops) {
        ...
        for(auto loop : loops) {
            printf("loop (entry, exit) = (B%d, B%d) has depth %d, is_innermost: %d\n", loop->header->index_in_procedure, loop->exit->index_in_procedure, loop->depth, loop->is_innermost);
        }
    ...
    void compute_function_loops(Procedure_IR *f) {
        ...
        for(auto bb : f->blocks) bb->belongs_to_loop = -1;
        for(auto bb : f->blocks) bb->loop_depth = 0;
        for(auto bb : f->blocks) bb->loop = NULL;
