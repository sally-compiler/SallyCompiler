...
#define twice 0
	...
	uint32 insert_into_basic_block(Basic_Block* basic_block, Value* v, bool insert_to_first) {
				...
				case BINARY_LESS_THAN: {
					if (curr->lhs->v->type == PHI && curr->lhs->v->b == bb) {
						if (curr->rhs->v->type == CONSTANT) {
							...
							Constant* bound = new Constant(((Constant*)curr->rhs->v)->value - 1);
						...
						if (curr->lhs->v->type == CONSTANT) {
							...
							Constant* bound = new Constant(((Constant*)curr->lhs->v)->value + 1);
						...
						could_unroll = 0;
						break;
					}//else maybe is constant operate constant  .. Value  out of loop
				}//slide into <=,i don't konw it
				case BINARY_LESS_THAN_OR_EQUAL_TO: {
					curr->op_type = BINARY_LESS_THAN_OR_EQUAL_TO;//because maybe slide  above
					if (curr->lhs->v->type == PHI && curr->lhs->v->b == bb) {//remove the third condition to below
						if (((Phi*)curr->lhs->v)->operands.len == 2) {
							...
							if (!the_same_control) {
								...
								for (auto k : val->operands) {
									if (k->v->type == CONSTANT && native_loop->body.find(k->v->b) == -1 && could_unroll != 2 && curr->rhs->v->type == CONSTANT) {
								...
								if (!the_same_control->high) {
									the_same_control->high = new_use(curr->rhs->v, curr);
								}
								else {
								...
								bool is_whole = false;
								for (auto k : val->operands) {
									if (k->v->type == CONSTANT && native_loop->body.find(k->v->b) == -1 && could_unroll != 2 && curr->rhs->v->type == CONSTANT) {///asknkcs??????????
										could_unroll = whole_unroll;
										is_whole = true;
									}
								}
						...
						if (((Phi*)curr->rhs->v)->operands.len == 2) {
							...
							if (!the_same_control) {
								...
								bool is_whole = false;
								for (auto k : val->operands) {
									if (k->v->type == CONSTANT && native_loop->body.find(k->v->b) == -1 && could_unroll != 2 && curr->lhs->v->type == CONSTANT) {
										could_unroll = whole_unroll;
										is_whole = true;
									}
								}
								...
								if (!the_same_control->low) {
									the_same_control->low = new_use(curr->lhs->v, curr);
								}
								else {
	...
	Basic_Block* dfs_copy(Basic_Block* standar, Loop* nat_loop, Procedure_IR* IR, bool another_while = false) {
		...
		for (auto val : standar->insts) {
			...
			case INST_BINARY: {
				...
				emit_mapping(((Instruction_Binary*)val)->lhs->v, nat_loop, another_while);
				emit_mapping(((Instruction_Binary*)val)->rhs->v, nat_loop, another_while);
				((Instruction_Binary*)vv)->lhs = new_use(mapping[((Instruction_Binary*)val)->lhs->v], vv);
				((Instruction_Binary*)vv)->rhs = new_use(mapping[((Instruction_Binary*)val)->rhs->v], vv);
				((Instruction_Binary*)vv)->op_type = ((Instruction_Binary*)val)->op_type;
				mapping[val] = vv;
			}break;
			case INST_UNARY: {
				...
				((Instruction_Unary*)vv)->op_type = ((Instruction_Unary*)val)->op_type;
				((Instruction_Unary*)vv)->oprend = new_use(mapping[s->oprend->v], vv);
				mapping[val] = vv;
			}break;
			case INST_RETURN: {
				...
				if (s->return_value) {
					((Instruction_Return*)vv)->return_value = new_use(mapping[s->return_value->v], vv);
				}
			}break;
			case INST_BRANCH: {
				vv = (Value*) new Instruction_Branch;
				auto s = ((Instruction_Branch*)val);
				emit_mapping(((Instruction_Branch*)val)->cond->v, nat_loop, another_while);
				mandatory_fix(s->cond->v);
				((Instruction_Branch*)vv)->cond = new_use(mapping[((Instruction_Branch*)val)->cond->v], vv);
				...
				if (nat_loop->body.find(s->false_target) == -1) {
					...
					((Instruction_Branch*)vv)->false_target = map_block[s->false_target];
				}
				mapping[val] = vv;
			}break;
			...
			case MEMORY_READ: {
				...
				if (s->offset) {
					...
					((Memory_Read*)vv)->offset = new_use(mapping[s->offset->v], vv);
				}
				else {
				...
				if (s->mem_ver) {
					...
					((Memory_Read*)vv)->mem_ver = new_use(mapping[s->mem_ver->v], vv);
				}
				else {
			...
			case MEMORY_WRITE: {
				...
				if (s->value_to_write) {
					...
					((Memory_Write*)vv)->value_to_write = new_use(mapping[s->value_to_write->v], vv);
				}
				else {
				...
				if (s->offset) {
					emit_mapping(s->offset->v, nat_loop, another_while);
					((Memory_Write*)vv)->offset = new_use(mapping[s->offset->v], vv);
				}
				else {
			...
			case FUNCTION_CALL: {
				...
				for (auto zhi : ((Function_Call*)val)->arguments) {
					...
					((Function_Call*)vv)->arguments.push(new_use(mapping[zhi->v], vv));
				}
				mapping[val] = vv;
			}break;
			}
			...
			for (int i = 0;i < dynamic_lates.len;i++) {
				if (val == dynamic_lates[i]->first) {
					...
					if (!another_while) {
						printf("change v%d :use[v%d  to v%d]\n", sign_variable[i]->user->n, sign_variable[i]->v->n, vv->n);
		...
		for (auto bb : standar->succs) {
			if (visited[bb] == false && nat_loop->body.find(bb) != -1) {
				...
				//connect_CFG(block, suc);//maybe this is useless ,computcfg will do 
			}
		}
		if (vv != NULL && vv->type == INST_DIRECT_BRANCH) {
			((Instruction_Direct_Branch*)vv)->target = map_block[((Instruction_Direct_Branch*)t)->target];
		}
		if (vv != NULL && vv->type == INST_BRANCH) {
			((Instruction_Branch*)vv)->false_target = map_block[((Instruction_Branch*)t)->false_target];
			((Instruction_Branch*)vv)->true_target = map_block[((Instruction_Branch*)t)->true_target];
		}
	...
	void seal_cfg(Loop* native_loop) {
		...
		for (auto p : unseal_phi) {
			...
			for (auto standar_us : ((Phi*)p->first)->operands) {
				if (p->first->b == native_loop->header) {
					if (native_loop->body.find(standar_us->v->b) == -1) {
						((Phi*)p->second)->operands.push(new_use(p->first, p->second));
						//map_block[native_loop->header->preds[0]] = native_loop->header;
					}
					else {
						auto ll = mapping[standar_us->v];
						((Phi*)p->second)->operands.push(new_use(mapping[standar_us->v], p->second));
					}
				}
				else {
					if (native_loop->body.find(standar_us->v->b) == -1) {
						((Phi*)p->second)->operands.push(new_use(standar_us->v, p->second));
						//map_block[native_loop->header->preds[0]] = native_loop->header;
					}
					else {
						auto ll = mapping[standar_us->v];
			...
			for (auto sou : ((Phi*)p->first)->sources) {
				if (p->first->b == native_loop->header && sou == native_loop->header->preds[0]) {
	...
	Basic_Block* emit_new_cfg(Basic_Block* head, Loop* native_loop, Procedure_IR* IR, bool another_while = false) {
		...
		for (int i = 0;i < dynamic_lates.len;i++) {
			if (!another_while) {
				printf("change v%d :use[v%d  to v%d]\n", sign_variable[i]->user->n, sign_variable[i]->v->n, dynamic_lates[i]->second->n);
	...
	void remake_cfg(Procedure_IR* IR, Loop* native_loop, int unroll_number, int num) {
		if (unroll_number == 0) {
			/* do nothing*/
		}
			...
			Basic_Block* b3 = emit_new_cfg(native_loop->header, native_loop, IR, true);
			auto midd = ((Instruction_Branch*)native_loop->header->insts.back())->false_target;

			((Instruction_Branch*)b3->insts.back())->false_target = midd;
			((Instruction_Branch*)native_loop->header->insts.back())->false_target = b3;
			...
			for (int i = 0;i < sign_variable.len;i++) {
				int index = 0;
				for (index = 0;index < ((Phi*)sign_variable[i]->user)->operands.len;index++) {
					if (((Phi*)sign_variable[i]->user)->operands[index]->v == sign_variable[i]->v) {
						Basic_Block* look = ((Phi*)sign_variable[i]->user)->sources[index];
						Basic_Block* watch = map_block[((Phi*)sign_variable[i]->user)->sources[index]];
			...
			connect_CFG(ex2, native_loop->header, true);//sasadascasaaxewasxxshuija;xscdjvhscjaox
				...
				for (auto val : b->insts) {
					...
					case INST_UNARY: {
						...
						if (native_loop->body.find(unary->oprend->v->b) != -1) {
							unary->oprend->remove();
                            unary->oprend = new_use(mapping[unary->oprend->v], unary);
						}
					}break;
					case FUNCTION_CALL: {
                        ...
                        for(int i = 0; i < function->arguments.len; i++) {
							...
							if (native_loop->body.find(us->v->b) != -1) {
								us->remove();
                                function->arguments[i] = new_use(mapping[us->v], function);
							}
                        }
					}break;
					case MEMORY_READ: {
						auto me_read = ((Memory_Read*)val);
						if (me_read->base != NULL && native_loop->body.find(me_read->base->v->b) != -1) {
							me_read->base->remove();
                            me_read->base = new_use(mapping[me_read->base->v], me_read);
						}
						if (me_read->offset != NULL && native_loop->body.find(me_read->offset->v->b) != -1) {
							me_read->offset->remove();
                            me_read->offset = new_use(mapping[me_read->offset->v], me_read);
						}
						if (me_read->mem_ver != NULL && native_loop->body.find(me_read->mem_ver->v->b) != -1) {
							me_read->mem_ver->remove();
                            me_read->mem_ver = new_use(mapping[me_read->mem_ver->v], me_read);
						}
					}break;
					case MEMORY_WRITE: {
						auto me_write = ((Memory_Write*)val);
						if (me_write->base != NULL && native_loop->body.find(me_write->base->v->b) != -1) {
							me_write->base->remove();
                            me_write->base = new_use(mapping[me_write->base->v], me_write);
						}
						if (me_write->offset != NULL && native_loop->body.find(me_write->offset->v->b) != -1) {
							me_write->offset->remove();
                            me_write->offset = new_use(mapping[me_write->offset->v], me_write);
						}
						if (me_write->value_to_write != NULL && native_loop->body.find(me_write->value_to_write->v->b) != -1) {
							me_write->value_to_write->remove();
                            me_write->value_to_write = new_use(mapping[me_write->value_to_write->v], me_write);
						}
					}break;
					case INST_RETURN: {
						auto ret = ((Instruction_Return*)val);
						if (ret->return_value != NULL && native_loop->body.find(ret->return_value->v->b) != -1) {
							ret->return_value->remove();
                            ret->return_value = new_use(mapping[ret->return_value->v], ret);
						}
					}break;
					default: {
	...
	void unroll_function(Procedure_IR* cfg) {
			...
			if (native_loop->is_innermost && !has_return_break) {
				...
				for (int i = 0;i < control.len;i++) {
					...
					for (auto p : ((Phi*)control[i]->variable)->operands) {
						if (native_loop->body.find(p->v->b) != -1) {//find phi's value in the loop
							if (p->v->type == INST_BINARY) {
								auto temp = (Instruction_Binary*)p->v;
								if (temp->lhs->v == control[i]->variable && temp->rhs->v->type == CONSTANT)//occur i=i op k;k=constant
								{
									switch (temp->op_type)
									{
									...
									next:
										...
										int times = 0;
										if (we_can_unroll == 2) {
											//need caliculate other high bound,no just one
											remake_cfg(cfg, native_loop, we_can_unroll, times);
											for (auto s : control[i]->high->v->b->insts) {//the upper bound of control variable 
												if (s == control[i]->high->v) {//find the declaration of upper bound 
													auto cons = new Constant(0);// bound - step * 3 +1
													cons->value = ((Constant*)temp->rhs->v)->value * 3 - 1;
													cons->b = control[i]->high->v->b;
													cons->n = curr_value_index++;
													control[i]->high->v->b->insts.insert(n + 1, cons);
													auto sub = new Instruction_Binary;
													sub->op_type = BINARY_SUBTRACT;
													sub->lhs = new_use(((Value*)control[i]->high->v), sub);//

													sub->rhs = new_use((Value*)cons, sub);
													control[i]->high->v->b->insts.insert(n + 2, sub);
													sub->b = control[i]->high->v->b;
													sub->n = curr_value_index++;
													if (control[i]->high->user->type == INST_BINARY) {//if is i< m or i <= m(m == bound),find the value
														((Instruction_Binary*)control[i]->high->user)->rhs->v = sub;//modify the bound to bound -step *3 +1
														//((Instruction_Binary*)control[i]->high->user)->comment ;//wait to modify comment
													}
												}
												n++;
											}
											...
											int min = compute_low_bound(control[i]->low);
											int max = compute_high_bound(control[i]->high);
											for (auto s : ((Phi*)control[i]->variable)->operands) {
												if (native_loop->body.find(s->v->b) == -1 && s->v->type == CONSTANT) {
													Constant* t = ((Constant*)s->v);
													if (t->value < min || t->value>max) {
														times = 0;
													}
													else {
														times = (max - t->value) / (((Constant*)temp->rhs->v)->value);
													}
												}
											}
											if (times > 60) {
										...
										bool is_add = true;
										post_process(cfg, native_loop, we_can_unroll);
										compute_CFG(cfg);
									}break;
									default: {
								...
								else if ((temp->lhs->v->type == CONSTANT && temp->rhs->v == control[i]->variable)) {
									switch (temp->op_type)
									{
									case BINARY_ADD: {//if op = +,i=i+2;
									next1:
											...
											for (auto s : ((Phi*)control[i]->variable)->operands) {
												if (native_loop->body.find(s->v->b) == -1 && s->v->type == CONSTANT) {
													...
													if (t->value < min || t->value>max) {
														...
														int mod = (max - t->value + 1) % (((Constant*)temp->lhs->v)->value);
														if (mod == 0) {
															times = (max - t->value + 1) / (((Constant*)temp->lhs->v)->value);
														}
														else {
															times = ((max - t->value + 1) / (((Constant*)temp->lhs->v)->value)) + 1;
														}
													}
												}
											}
		...
		printf("ending unrolling_loop-------------------------------------------------------------------\n");

	}
}

OPT_PASS(loop_unrolling) {
	run_on_every_function(IR, UNROLL::unroll_function);
}
