...
int32 evaluate(Ast_Block* block, Ast_Node* node, Array<Ast_Declaration *> globals) {
    switch (node->type) {
        ...
        case Ast_Type_Array_Reference: {
            ...
            auto init_seq = get_init_sequence(arr_dims, (Ast_Init_List *) decl->initial_value);
            ...
            for(auto init_inst : init_seq) {
                if (init_inst.tag == LONG) {
                    ...
                    if (offset >= curr_index && offset < curr_index + init_inst.num_of_zeros) {
                        return 0;
                    }
        ...
        case Ast_Type_Binary_Op: {
            ...
            switch (binary_node->op) {
                ...
                case BINARY_SUBTRACT:                  return lhs - rhs;
                case BINARY_MULTIPLY:                  return lhs * rhs;
                case BINARY_DIVIDE:                    return lhs / rhs;
                case BINARY_MOD:                       return lhs % rhs;
                case BINARY_LOGICAL_AND:               return lhs && rhs;
                case BINARY_LOGICAL_OR:                return lhs || rhs;
                case BINARY_LESS_THAN:                 return lhs < rhs;
                case BINARY_GREATER_THAN:              return lhs > rhs;
                case BINARY_LESS_THAN_OR_EQUAL_TO:     return lhs <= rhs;
                case BINARY_GREATER_THAN_OR_EQUAL_TO:  return lhs >= rhs;
                case BINARY_NOT_EQUAL_TO:              return lhs != rhs;
                case BINARY_EQUAL_TO:                  return lhs == rhs;
                default: { exit(11); assert(false); return 0; }
            }
        ...
        case Ast_Type_Unary_Op: {
            ...
            switch (unary_node->op) {
                case UNARY_NOT: { return !operand; }  break;
                case UNARY_POSITIVE: { return operand; }  break;
                case UNARY_NEGATIVE: { return -operand; } break;
            }

        } break;

        default: { exit(9); report_error(node, "request a complete expression!"); }
    }
...
Ast_Block *Parser::parse_block(Ast_Block *parent_block, bool32 is_procedure_block, Ast_Block *new_block) {
    
    if(!new_block) new_block = AST_NEW(Ast_Block);
    ...
    if (tokenizer.token()->type == '{' || is_procedure_block) {
        ...
        // @HACK what is this
        if (tokenizer.token()->type == ';') tokenizer.eat();
    }
...
Type_Info *Parser::parse_basic_type(bool32 return_type) {
    ...
    switch (token_type) {
        case TOKEN_KEYWORD_INT:   { data_type = TYPE_NEW(Type_Info_Integer); } break;
        case TOKEN_KEYWORD_VOID:  { 
            if (return_type) {
                data_type = TYPE_NEW(Type_Info_Void); break;
            }
        }
        default: { report_error(tokenizer.token(), "expected a type, got %d", tokenizer.token()->type); }
    }
    tokenizer.eat();
    return data_type;
}
...
// int a[5];
// int a[5][4] = {1};
Ast_Declaration_List *Parser::parse_declaration(Ast_Block *scope, Array<Ast_Declaration *> *decls) {
    ...
    while(tokenizer.token()->type != ';') {
        ...
        if(check_for_redeclaration(*decls, var_decl->id->name)) {
            report_error((Ast_Node *)var_decl, "variable '%s' has been already declared", var_decl->id->name);
        }
        */

        if (var_decl->is_const) tokenizer.expect('=');
        ...
        if (tokenizer.token()->type == '=') {
            ...
            if (tokenizer.token()->type == '{') {
                ...
                if (is_const) {
                    int32 const_val = evaluate(scope, var_decl->initial_value, program->globals);
        ...
        // @TODO: handle error
        } else if (tokenizer.token()->type == ';' || tokenizer.token()->type == ',') {
            // variable with no initial value
            var_decl->initial_value = NULL;
        } else {
            report_error((Ast_Node *)var_decl, "expected comma, semicolon or an initial value for variable '%s'", var_decl->id->name);
        }
...
Ast_Node *Parser::parse_atom(Ast_Block *scope) {
    ...
    if (t->type == '(') {
        ...
        if (t->type == '-' && tokenizer.peek()->type == TOKEN_INTEGER) {
            ...
            auto int_node = AST_NEW(Ast_Integer_Literal);
            int_node->value = -(t->data.int_val); // @NOTE! token->data.int_val is int64 to work with INT_MIN
            ...
            switch (t->type){
                case '+': unary_node->op = UNARY_POSITIVE; break;
                case '-': unary_node->op = UNARY_NEGATIVE; break;
                case '!': unary_node->op = UNARY_NOT; break;
                default: report_error(t, " Having not this operator %s!\n", token_to_desc(t));
            }
...
Ast_Node *Parser::parse_expr(Ast_Block *scope, int min_prec) {
    ...
    while(1) {
        ...
        if (lhs == NULL || !is_expr(lhs) || !t->is_op) break;
        ...
        switch(t->type) {
            case '+': { op_type = BINARY_ADD;      prec = 12; assoc = LEFT; } break;
            case '-': { op_type = BINARY_SUBTRACT; prec = 12; assoc = LEFT; } break;
            case '*': { op_type = BINARY_MULTIPLY; prec = 13; assoc = LEFT; } break;
            case '/': { op_type = BINARY_DIVIDE;   prec = 13; assoc = LEFT; } break;
            case '%': { op_type = BINARY_MOD;      prec = 13; assoc = LEFT; } break;
            case TOKEN_LOGICAL_AND:              { op_type = BINARY_LOGICAL_AND;                 prec = 9;  assoc = LEFT; } break;
            case TOKEN_LOGICAL_OR:               { op_type = BINARY_LOGICAL_OR;                  prec = 8;  assoc = LEFT; } break;
            case TOKEN_LESS_THAN:                { op_type = BINARY_LESS_THAN;                   prec = 11; assoc = LEFT; } break;
            case TOKEN_GREATER_THAN:             { op_type = BINARY_GREATER_THAN;                prec = 11; assoc = LEFT; } break;
            case TOKEN_LESS_THAN_OR_EQUAL_TO:    { op_type = BINARY_LESS_THAN_OR_EQUAL_TO;       prec = 11; assoc = LEFT; } break;
            case TOKEN_GREATER_THAN_OR_EQUAL_TO: { op_type = BINARY_GREATER_THAN_OR_EQUAL_TO;    prec = 11; assoc = LEFT; } break;
            case TOKEN_NOT_EQUAL_TO:             { op_type = BINARY_NOT_EQUAL_TO;                prec = 10; assoc = LEFT; } break;
            case TOKEN_EQUAL_TO:                 { op_type = BINARY_EQUAL_TO;                    prec = 10; assoc = LEFT; } break;

            default: { not_a_binary_op = true; }
        }

        if (not_a_binary_op || prec < min_prec) break;
        ...
        if (!is_expr(rhs)) {
            report_error(t, "expected an expression on the right hand side of %s", token_to_desc(t));
        }
    ...
    if (tokenizer.token()->type == '=') {
        ...
        if (lhs->type != Ast_Type_Identifier && lhs->type != Ast_Type_Array_Reference) {
            report_error((Ast_Node *)assign_node, "expected an identifier or array reference on the left of assignment");
        }
        ...
        if(!decl) {
            report_error((Ast_Node *)assign_node, "undefined variable '%s'", ident->name);
        }

        if (decl->is_const) {
            report_error((Ast_Node*)assign_node, "assigning to a constant value '%s'", ident->name);
        }
        ...
        if(!assign_node->value_to_assign) {
            ...
            report_error((Ast_Node *)assign_node, "expected an value to assign for variable '%s'", ident->name);
        }
...
bool32 Parser::is_expr(Ast_Node *node) {
    ...
    return (
            typ == Ast_Type_Identifier ||
            typ == Ast_Type_Array_Reference ||
            typ == Ast_Type_Integer_Literal ||
            typ == Ast_Type_Binary_Op ||
            typ == Ast_Type_Unary_Op ||
            typ == Ast_Type_Procedure_Call ||
...
Ast_Statement *Parser::parse_statement(Ast_Block *block) {
    ...
    if (!stm->body) return NULL;

    // Can only use break and continue inside loops
    if (!inside_loop) {
        if (stm->body->type == Ast_Type_Continue || stm->body->type == Ast_Type_Break) {
    ...
    if (stm->body->type != Ast_Type_Block && 
        stm->body->type != Ast_Type_If &&
...
Program_AST *Parser::parse(const char* filename) {
    ...
    // int foo[2] = 3;
    while(1) {
        if (tokenizer.token()->type == TOKEN_END) break;
        if (tokenizer.peek(2)->type == '(') {
            ...
            if (true) {
                Ast_Statement *prev = NULL, *stmt = proc_decl->proc_body->first_statement;
                ...
                if (prev->body == NULL || prev->body->type != Ast_Type_Procedure_Return) {
...
Ast_Declaration *check_for_redeclaration(Array<Ast_Declaration *> decls, const char *name) {
    for(int decl_index = 0;
            decl_index < decls.len;
            decl_index++) {
...
Ast_Declaration *find_declaration(Ast_Block *block, Ast_Identifier *id, Array<Ast_Declaration *> *globals) {
...
Ast_Declaration *find_declaration(Ast_Block *block, const char *name, Array<Ast_Declaration *> *globals) {
    Ast_Declaration* decl = NULL;
    while(block) {
...
const char *type_info_to_desc(Type_Info *info) {
    switch(info->tag) {
        ...
        case TYPE_PROCEDURE: { return "procedure"; } break;
