...
void condify(Program_Asm *prog) {
    for (auto f : prog->functions) {
        for (auto bb : f->mbs) {
            ...
            if (if_end->preds.len != 2)
                continue;
            if (then_bb->preds.len != 1)
                continue;
            if (then_bb->succs.len != 1)
                continue;
            if (then_bb->succs[0] != if_end)
                continue;

            if (!can_condify(then_bb))
                continue;

            if (if_end->i != bb->i + 1 && if_end->i != bb->i + 2)
                continue;
            if (then_bb->i != bb->i + 1 && then_bb->i != bb->i + 2)
                continue;
            ...
            for (auto I = then_bb->inst; I != then_bb->control_transfer_inst;
                 I = I->next) {
                I->cond = br->cond;
            }
