...
namespace RA {
            ...
            for(auto I = func_asm->mbs[b->i]->last_inst; I; I=I->prev) {
                ...
                for(auto def : defs) {
                    if (need_alloc(def)) {
                        ...
                        for(auto l : live) add_edge(l, def);
                    }
                }
    ...
    void coalesce() {
        ...
        //printf("coalescing "); print_operand(m->dst); printf(" <- "); print_operand(m->src); printf("\n");
    ...
    void assign_colors() {
        while(!select_stack.empty()) {
            auto n = select_stack.back(); select_stack.pop();
    ...
    void select_spill() {
        ...
        for(auto it = spill_worklist.begin();
            ...
            if (in(*it, already_spilled)) continue;
    ...
    void allocate_register(Func_Asm *func_asm) {
        ...
        */
        ...
        do {
            ...
            else if (!spill_worklist.empty()) select_spill();
        } while(
        ...
        if (!spilled_nodes.empty()) {
            ...
            for (MOperand n : spilled_nodes) {
                ...
                printf("spilling nodes: "); print_operand(n); printf("\n");
            ...
            if (!done_post_work) {
                    ...
                    for(auto I=bb->inst; I; I=I->next) {
                        ...
                        if (I->tag == MI_STORE) {
                            ...
                            if (str->mem_tag == MEM_SAVE_SPILL) {
                                int32 offset_value = ((str->offset.tag == SHAYEBUSHI) ? 0 : str->offset.value);
                                int32 offset_relative_to_sp = func_asm->stack_size + offset_value;
                                str->base.value = sp;
                ...
                if (need_to_legalize_imm) {
                    ...
                    for(uint8 r : needs_save) last_needs_save.push(r);
