...
void emit_load_of_global_ref(Func_Asm *func_asm, Global *v, MOperand vreg,  Machine_Block *mb) {
...
void emit_binary_value_naive(Instruction_Binary *bi_I, Machine_Block *mb) {
    if (bi_I->op_type == BINARY_MOD)
    {
    ...
    } else if (bi_I->op_type >= BINARY_NOT_EQUAL_TO && bi_I->op_type <= BINARY_GREATER_THAN) {
        ...
        if (bi_I->op_type == BINARY_MULTIPLY || bi_I->op_type == BINARY_DIVIDE)
            rhs = make_operand(bi_I->rhs->v, mb, /* no imm for mul and sdiv */ true);
        else
...
void emit_binary_value(Instruction_Binary *bi_I, Machine_Block *mb) {
    if (!opt) return emit_binary_value_naive(bi_I, mb);
...
Program_Asm *emit_asm(Program_IR *program_IR, bool enable_optimization) {
    ...
    for(auto func_IR : program_IR->procedures) {
        if (func_IR->is_builtin) continue;
...
void emit_branch_asm(Func_Asm *func_asm, Binary_Op_Type op, Instruction_Branch *br_I, Machine_Block *mb) {
...
void emit_relational_branch(Instruction_Binary *bi, Instruction_Branch *br, Machine_Block *mb) {
...
void emit_unary_branch(Instruction_Unary *un, Instruction_Branch *br, Machine_Block *mb) {

    if (un->op_type == UNARY_NEGATIVE) {
        auto cmp = new MI_Compare(make_operand(un->oprend->v, mb, true), make_imm(0));
        push(cmp, mb);
...
void emit_legacy(Value *I, Basic_Block *bb, Machine_Block *mb, int32 &i) {
    switch(I->type) {
        case INST_BRANCH: {
            ...
            auto cmp = new MI_Compare(make_operand(br_I->cond->v, mb, true), make_imm(0));
            push((MI *)cmp, mb);
        ...
        case INST_UNARY: {
            ...
            if (un_I->op_type == UNARY_NEGATIVE) {
                ...
                if (next_inst->type == INST_BRANCH) {
                    auto cmp = new MI_Compare(make_operand(un_I->oprend->v, mb, true), make_imm(0));
                    push((MI *)cmp, mb);
                    i++; // handle the branch IR at the same time

                    emit_branch_asm(func_asm, BINARY_NOT_EQUAL_TO, (Instruction_Branch *)next_inst, mb);
                } else {
                    auto rsb = new MI_Binary();
                    rsb->op = BINARY_RSB;
                ...
                if (next_inst->type == INST_BRANCH) {
                    ...
                    emit_branch_asm(func_asm, BINARY_EQUAL_TO, (Instruction_Branch *)next_inst, mb);
        ...
        case INST_BINARY: {
            ...
            if (next_inst->type != INST_BRANCH) {
                ...
                if (bi_I->op_type >= BINARY_NOT_EQUAL_TO && bi_I->op_type <= BINARY_GREATER_THAN) {
                    auto lhs = make_operand(bi_I->lhs->v, mb, true);
                    auto rhs = make_operand(bi_I->rhs->v, mb);
                    auto cmp = new MI_Compare(lhs, rhs);
        ...
        for(int i = 0; i < bb->insts.len; i++) {
            ...
            switch(I->type) {
                ...
                case INST_BINARY: {
                    ...
                    if (br && br->cond->v == I) {
                        ...
                        if (is_relational_binary_op(bi_I->op_type)) {
                            ...
                            auto cmp = new MI_Compare(make_operand(bi_I, mb), make_imm(0));
                ...
                case INST_DIRECT_BRANCH: {
                    ...
                    auto target_mb = func_asm->mbs[br_I->target->index_in_procedure];
        ...
        for(int i = 0; i < bb->insts.len; i++) {
            if (bb->insts[i]->type == PHI) {
                ...
                for(int p = 0; p < phi->operands.len; p++) {
                    ...
                    auto mv = emit_move(incoming, make_operand(phi_opr->v, pred_mb));
...
void build_globals(String_Builder *s, Array<Ast_Declaration *> globals) {
            ...
            if (init_list && init_list->list.len != 0) {
                ...
                for (int j = 0; j < init_inst.len; j++)
                {
                    ...
                    if (init_inst[j].tag == LONG) {
                        int32 c = evaluate(NULL, init_inst[j].init_value, globals);
                        s->append(".word  %d\n", c);
                    } else {
    ...
    for (int i = 0; i < globals.len; i++) {
		if (globals[i]->decl_type->tag == TYPE_ARRAY) {
			auto dims = ((Type_Info_Array *)globals[i]->decl_type)->dimension;
			auto init_list = (Ast_Init_List *)globals[i]->initial_value;
            if (init_list && init_list->list.len != 0) continue;

            init_list = new Ast_Init_List;

            int num_of_zeros = get_init_sequence(dims, init_list)[0].num_of_zeros*4;
            s->append("%s:\n", globals[i]->id->name);
            s->append("    .space %d\n", num_of_zeros);
...
void build_operand(String_Builder *s, MOperand op) {
    switch(op.tag) {
        case REG:   {
            if (op.value == fp) s->append("fp");
            else if (op.value == sp) s->append("sp");
            else if (op.value == lr) s->append("lr");
            else if (op.value == pc) s->append("pc");
            else s->append("r%d", op.value); 
        } break;

        case VREG:  { s->append("vr%d", op.value); } break;
        case IMM:   { s->append("#%d", op.value); } break;
		case ADR_GLOBAL: { s->append("%s", op.adr); } break;
		case SHAYEBUSHI: { exit(52); assert(false); } break;

    }
	switch(op.s_tag) {
	case Nothing: break;
	case LSL: { s->append(", lsl #%d", op.s_value); } break;
	case LSR: { s->append(", lsr #%d", op.s_value); } break;
	case ASL: { s->append(", asl #%d", op.s_value); } break;
	case ASR: { s->append(", asr #%d", op.s_value); } break;
	}
}
...
void build_function_asm(String_Builder *s, Func_Asm *func) {
    ...
    for(int i = 0; i < func->mbs.len; i++) {
        ...
        if (i != func->mbs.len-1) next_bb = func->mbs[i+1];
        ...
        for(auto I = func->mbs[i]->inst; I; I=I->next) {
            ...
            switch(I->tag) {
                ...
                case MI_BINARY: {
                    ...
                    switch(bi->op) {
                        case BINARY_ADD:         { s->append("add"); } break;
                        case BINARY_SUBTRACT:    { s->append("sub"); } break;
                        case BINARY_MULTIPLY:    { s->append("mul"); } break;
                        case BINARY_DIVIDE:      { s->append("sdiv"); } break;
            			case BINARY_LSL:         { s->append("lsl");  } break;
            			case BINARY_LSR:         { s->append("lsr");  } break;
            			case BINARY_ASL:         { s->append("asl");  } break;
            			case BINARY_ASR:         { s->append("asr");  } break;
            			case BINARY_RSB:         { s->append("rsb");  } break;
						case BINARY_BITWISE_AND: { s->append("and"); }break;
						case BINARY_BITWISE_OR:  { s->append("orr"); }break;
						case BINARY_BIC:         { s->append("bic"); }break;
						case BINARY_SMMUL:       { s->append("smmul"); }break;
                        default:                 { exit(53); assert(false && "unknown binary asm instruction."); }
                    }
                ...
                case MI_POP: {
                    ...
                    for (int i = 0; i < push_or_pop->operands.len; i++) {
                        ...
                        if (i != push_or_pop->operands.len-1) s->append(", ");
                    }
                    s->append("}");
                ...
                case MI_STORE: {
					...
					if (load_or_store->base.tag == ADR_GLOBAL) {
						s->append("movw%s ", cond); build_operand(s, load_or_store->reg); 
						s->append(", ");
						s->append(":lower16:"); build_operand(s, load_or_store->base);
						s->append("\n    movt%s ", cond); build_operand(s, load_or_store->reg);
						s->append(", ");
						s->append(":upper16:"); build_operand(s, load_or_store->base);
					}
					else if (load_or_store->base.tag == IMM) {
						auto val = load_or_store->base.value;
						if (can_be_imm_ror(val))
						{
							s->append("mov%s ", cond); build_operand(s, load_or_store->reg);
							s->append(", ");
						...
						{
							s->append("mvn%s ", cond); build_operand(s, load_or_store->reg);
							s->append(", ");
							auto valn = -val - 1;
